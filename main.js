...
function searchAll(query) {
  ...
  document.querySelectorAll('.welcome-card').forEach(card => {
    if (card.textContent.toLowerCase().includes(query) && query.length > 0) {
      card.style.display = '';
  ...
  document.querySelectorAll('.section').forEach(sec => {
    if (sec.textContent.toLowerCase().includes(query) && query.length > 0) {
      sec.style.display = '';
  ...
  document.querySelectorAll('#recipes h3').forEach(h3 => {
    if (h3.textContent.toLowerCase().includes(query) && query.length > 0) {
      h3.style.display = '';
  ...
  if (hidden) {
    if (hidden.textContent.toLowerCase().includes(query) && query.length > 0) {
  ...
  // 如果没有输入，恢复全部显示
  if (!query) {
    document.querySelectorAll('.welcome-card, .section, #recipes h3').forEach(el => el.style.display = '');
  ...
  if (firstMatch && typeof firstMatch.scrollIntoView === 'function') {
    setTimeout(() => {
      ...
      // 高亮
      firstMatch.classList.add('quantum-highlight');
      setTimeout(() => firstMatch.classList.remove('quantum-highlight'), 1200);
    }, 200);
  }
}
