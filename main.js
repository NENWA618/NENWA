...
if (elements.searchInput && elements.searchBar) {
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', () => {
      ...
      // 搜索收入
      const filteredIncome = budget.incomeRecords.filter(r =>
        (r.source && r.source.toLowerCase().includes(keyword)) ||
        (r.category && r.category.toLowerCase().includes(keyword)) ||
        String(r.amount).includes(keyword) ||
        (r.date && r.date.includes(keyword))
      );
      // 搜索支出
      const filteredExpense = budget.expenseRecords.filter(r =>
        (r.name && r.name.toLowerCase().includes(keyword)) ||
        (r.category && r.category.toLowerCase().includes(keyword)) ||
        String(r.price).includes(keyword) ||
        String(r.quantity).includes(keyword) ||
        (r.date && r.date.includes(keyword))
      );
      renderFilteredTables(filteredIncome, filteredExpense);
    });
