...
function searchAll(query) {
  ...
  document.querySelectorAll('.section').forEach(sec => {
    sec.style.display = sec.textContent.toLowerCase().includes(query) ? '' : '';
  });
  // 3. 食谱
  document.querySelectorAll('#recipes h3').forEach(h3 => {
    h3.style.display = h3.textContent.toLowerCase().includes(query) ? '' : 'none';
  });
  // 4. 隐藏内容
  ...
  if (hidden) {
    if (hidden.textContent.toLowerCase().includes(query) && query.length > 0) {
      ...
      setTimeout(() => { hidden.style.display = 'none'; }, 3000);
    } else {
...
if (elements.searchInput) {
  elements.searchInput.addEventListener('input', function (e) {
    if (!this.value.trim()) {
      // 恢复全部显示
      document.querySelectorAll('.welcome-card').forEach(card => card.style.display = '');
      document.querySelectorAll('.section').forEach(sec => sec.style.display = '');
      document.querySelectorAll('#recipes h3').forEach(h3 => h3.style.display = '');
      const hidden = document.querySelector('.hidden-content');
